...
def _render_markdown(
    ...
    lines += [
        ...
        "| Framework | LangGraph StateGraph Orchestration |",
    ...
    # ── AI Clinical Reasoning Section (Ollama LLM) ────────────────────────────
    if llm_sections and any(llm_sections.values()):
        lines += [
            "## 7. AI Clinical Reasoning (Ollama LLM)",
            "",
            "> The following commentary was generated by a local Ollama LLM "
            "(llama3.2:3b) for each agent in the pipeline. It provides AI-assisted "
            "clinical reasoning to supplement the structured analysis above.",
            "",
        ]
        section_map = [
            ("Patient Intake Commentary", llm_sections.get("intake", "")),
            ("Diagnostic Reasoning", llm_sections.get("symptom", "")),
            ("Treatment Safety Commentary", llm_sections.get("treatment", "")),
            ("Documentation Review", llm_sections.get("report", "")),
        ]
        for heading, content in section_map:
            if content:
                lines += [
                    f"### 7.{section_map.index((heading, content)) + 1} {heading}",
                    "",
                    content,
                    "",
                ]
